...
public class SimpleConnectionPoolDataSource implements DataSource {
    ...
    public SimpleConnectionPoolDataSource(String configFile)
        ...
        url = props.getProperty("classdemo.ch16.JDBCDemo.url");
        username = props.getProperty("classdemo.ch16.JDBCDemo.username");
        password = props.getProperty("classdemo.ch16.JDBCDemo.password");
        max = Integer.parseInt(props.getProperty("classdemo.ch16.JDBCDemo.poolmax"));

        conns = Collections.synchronizedList(new ArrayList<>());
    }

    public synchronized Connection getConnection() throws SQLException {
        if(conns.isEmpty()) { //如果List為空 就建立新的ConnectionWrapper
    ...
    private class ConnectionWrapper implements Connection {
        ...
        public void close() throws SQLException {
            if(conns.size() == max) { //如果超出最大可維護數量 就關閉connection
                conn.close();
            }
            else {
                conns.add(this); //否則放入List中備用
            }
        }
